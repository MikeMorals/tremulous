...
bool voidFunction2(itemDef_t *var1, int var2) { return false; }
...
void *UI_Alloc(int size)
{
    ...
    if (allocPoint + size > MEM_POOL_SIZE)
    {
        ...
        // DC->Com_Printf(S_COLOR_YELLOW"WARNING: UI Out of Memory!\n");
        return NULL;
    }
...
bool UI_OutOfMemory() { return outOfMemory; }
...
=================
*/
__attribute__((format(printf, 2, 3))) void PC_SourceWarning(int handle, const char *format, ...)
{
    int line;
    char filename[128];
    va_list argptr;
...
=================
*/
__attribute__((format(printf, 2, 3))) void PC_SourceError(int handle, const char *format, ...)
{
    int line;
    char filename[128];
    va_list argptr;
...
=================
*/
void UI_ClearClipRegion(void) { re->SetClipRegion(NULL); }
...
static bool Item_ComboBox_MaybeCastToListBox(itemDef_t *item)
{
    listBoxDef_t *listPtr = item->typeData.list;
    bool cast = g_comboBoxItem != NULL && (item->type == ITEM_TYPE_COMBOBOX);
...
static void Item_ComboBox_MaybeUnCastFromListBox(itemDef_t *item, bool unCast)
{
    if (unCast)
    {
...
bool Item_ComboBox_HandleKey(itemDef_t *item, int key, bool down, bool force)
{
    if (g_comboBoxItem != NULL)
    {
        bool result;

        bool cast = Item_ComboBox_MaybeCastToListBox(item);
...
bool Item_Cycle_HandleKey(itemDef_t *item, int key)
{
...
static void Scroll_ListBox_AutoFunc(void *p)
{
    ...
    bool cast = Item_ComboBox_MaybeCastToListBox(si->item);
    ...
    switch (item->type)
    {
        ...
        case ITEM_TYPE_COMBOBOX:
        {
            ...
            if (flags & (WINDOW_LB_UPARROW | WINDOW_LB_DOWNARROW))
            {
                ...
                scrollInfo.scrollDir = (flags & WINDOW_LB_UPARROW) ? true : false;
...
bool Item_HandleKey(itemDef_t *item, int key, bool down)
{
    ...
    switch (item->type)
    {
        ...
        case ITEM_TYPE_COMBOBOX:
            return Item_ComboBox_HandleKey(item, key, down, false);
...
static bool cacheCreationFailed = false;
...
static bool UI_NextWrapLine(const char **text, float *x, float *y)
{
...
void Item_Text_Wrapped_Paint(itemDef_t *item)
{
    ...
    float x, y, w, h;
    vec4_t color;
    bool useWrapCache = (bool)DC->getCVarValue("ui_textWrapCache");
...
bool Display_KeyBindPending(void) { return g_waitingForKey; }
...
void Item_ListBoxRow_Paint(itemDef_t *item, int row, int renderPos, bool highlight, bool scrollbar)
{
    float x, y, w;
    listBoxDef_t *listPtr = item->typeData.list;
...
void Item_ListBox_Paint(itemDef_t *item)
{
    ...
    listBoxDef_t *listPtr = item->typeData.list;
    int count = DC->feederCount(item->feederID);
    bool scrollbar = !(listPtr->noscrollbar && count > Item_ListBox_NumItemsForItemHeight(item));
    if (scrollbar)
    {
    ...
    for (i = listPtr->startPos; i < listPtr->endPos; i++)
        Item_ListBoxRow_Paint(item, i, i - listPtr->startPos, true, scrollbar);
}
...
void Item_ComboBox_Paint(itemDef_t *item)
{
    ...
    Item_ListBoxRow_Paint(item, item->cursorPos, 0, false, true);
    if (g_comboBoxItem != NULL)
    {
        bool cast = Item_ComboBox_MaybeCastToListBox(item);
        Item_Paint(item);
...
void Menu_ScrollFeeder(menuDef_t *menu, int feeder, bool down)
{
    if (menu)
    {
        ...
        for (i = 0; i < menu->itemCount; i++)
        {
            ...
            {
                bool cast = Item_ComboBox_MaybeCastToListBox(item);
                Item_ListBox_HandleKey(item, down ? K_DOWNARROW : K_UPARROW, true, true);
                Item_ComboBox_MaybeUnCastFromListBox(item, cast);
...
static bool Item_HandleMouseMove(itemDef_t *item, float x, float y, int pass, bool focusSet)
{
...
void KeywordHash_Add(keywordHash_t *table[], keywordHash_t *key)
{
    ...
    /*
       if(table[hash])     int collision = true;
       */
    key->next = table[hash];
    table[hash] = key;
}
...
// aspectBias <bias>
bool ItemParse_aspectBias(itemDef_t *item, int handle)
{
...
bool ItemParse_notselectable(itemDef_t *item, int handle)
{
    ...
    item->typeData.list->notselectable = true;
    return true;
}

// noscrollbar
bool ItemParse_noscrollbar(itemDef_t *item, int handle)
{
    (void)handle;

    item->typeData.list->noscrollbar = true;
    return true;
}

// resetonfeederchange
bool ItemParse_resetonfeederchange(itemDef_t *item, int handle)
{
    (void)handle;

    item->typeData.list->resetonfeederchange = true;
    return true;
}
...
bool ItemParse_textvalign(itemDef_t *item, int handle)
{
...
bool ItemParse_onTextEntry(itemDef_t *item, int handle)
{
...
bool ItemParse_maxFieldWidth(itemDef_t *item, int handle)
{
...
bool MenuParse_aspectBias(itemDef_t *item, int handle)
{
    menuDef_t *menu = (menuDef_t *)item;
