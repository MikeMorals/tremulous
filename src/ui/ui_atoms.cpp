...
=================
*/
void UI_StartDemoLoop(void) { Cbuf_ExecuteText(EXEC_APPEND, "d1\n"); }
...
static void UI_Me_f(void)
{
    ...
    Cbuf_ExecuteText(EXEC_APPEND, va("say \"/me %s\"\n", buf));
}
...
void UI_DrawNamedPic(float x, float y, float width, float height, const char *picname)
{
    ...
    hShader = re->RegisterShaderNoMip(picname);
...
void UI_DrawHandlePic(float x, float y, float w, float h, qhandle_t hShader)
{
    ...
    re->DrawStretchPic(x, y, w, h, s0, t0, s1, t1, hShader);
}
...
void UI_FillRect(float x, float y, float width, float height, const float *color)
{
    ...
    UI_AdjustFrom640(&x, &y, &width, &height);
    re->DrawStretchPic(x, y, width, height, 0, 0, 0, 0, uiInfo.uiDC.whiteShader);

    re->SetColor(NULL);
}

void UI_SetColor(const float *rgba) { re->SetColor(rgba); }
